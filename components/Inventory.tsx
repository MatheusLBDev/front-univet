...
const NewSaleTab: React.FC<InventoryProps> = ({ products, services, fetchData }) => {
    ...
    const addToCart = (item: Product | Service, type: 'product' | 'service') => {
        const existing = cart.find(cartItem => cartItem.id === item.id && cartItem.type === type);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                ...
                <ul className="divide-y mb-4">{cart.length > 0 ? cart.map(item => <li key={item.id + item.type} className="py-2 flex justify-between"><span>{item.quantity}x {item.name}</span><span>{(item.price * item.quantity).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></li>) : <p className="text-gray-500">O carrinho está vazio.</p>}</ul>
